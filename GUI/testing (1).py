...
from plan_report import Report, ReferenceGroup
...
def update_match(event: str, values: Values,
    ...
    if not selection:
        return history
    plan_item = selection[0]
    if 'ENTER' in event:
        new_value.plan_Item = enter_value(ref.reference_name)
    else:
    ...
    tree.Update(key=lookup, value=new_value, icon=changed_icon)
    return history
